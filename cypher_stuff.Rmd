...
```{r, only_ec_byyear}
  ...
  mutate(date_code = as.Date(date, format = "%d/%m/%Y")) %>% 
  group_by(title) %>% 
  summarise(total_people = sum(people), date_code = mean(date_code))

ggplot(aa, aes(x = date_code, y = total_people)) + 
  geom_point(alpha = 0.5) + 
  geom_smooth() +
  scale_y_log10()
