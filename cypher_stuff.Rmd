...
ggplot(count_table, aes(x = people, y = count)) + geom_bar(stat = 'identity')+ facet_wrap(~instrument, scales = 'free')
...
mass_award <- all_titles %>% group_by(title, instrument) %>% filter(n() > 5) %>% summarise(count=n())
