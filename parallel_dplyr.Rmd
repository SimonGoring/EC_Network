...
[The original script](https://github.com/SimonGoring/EC_Network/commit/fcf62c0ea0305ec941a0e19f5d2d338d3f285b9f) ran in two steps.  The first step simply read in the files.  There were over 400,000 files in the NSF dataset, each representing a single award to a single institution.  The awards were read in by chaining `read_xml()` and `as_list()` from the `xml2` package.  This then produced a long list of elements that could be processed.  From there I used `purrr`'s `map()` function to apply `read_award()` and then the `parse_award()` function to each filename.
